...
class TwoHandsManipulateEnvBBeckman(hand_env.TwoHandsEnvBBeckman,
    ...
    def _get_achieved_goal(self):
        ...
        assert object_qpos.shape == (7,)
        return object_qpos
        # [[[ bbeckman: can't return the following till we fix up rewards etc. ]]]
    ...
    def _get_obs(self):
        ...
        # are looking at the screen. Likewise for "right." ]]]
        robot_left_qpos, robot_left_qvel = robot_get_obs(self.sim, "robot0")
        assert robot_left_qpos.shape == robot_left_qvel.shape == (24,)
        ...
        # code because it might be confusing.
        robot_rigt_qpos, robot_rigt_qvel = robot_get_obs(self.sim, "robot1")
        assert robot_rigt_qpos.shape == robot_rigt_qvel.shape == (24,)
        ...
        # they use the word "object" so as to be less specific. ]]]
        object_left_qpos = self.sim.data.get_joint_qpos('object:joint')
        assert object_left_qpos.shape == (7,)
        ...
        assert object_rigt_qpos.shape == (7,)
        ...
        # [[[ bbeckman: old code here ]]]
        both_robots_qpos, robot_qvel = robot_get_obs(self.sim)
        assert(both_robots_qpos.shape == (48,))
        assert(robot_qvel.shape == (48,))
        achieved_goal = self._get_achieved_goal().ravel()  # this contains the object position + rotation
        observation = np.concatenate([both_robots_qpos, robot_qvel, object_left_qvel, object_rigt_qvel, achieved_goal])
...
class TwoHandsBlockEnvBBeckman(TwoHandsManipulateEnvBBeckman):
    """[[[ bbeckman: A copy so as not to derange the original. A
