...
class TwoHandsManipulateEnvBBeckman(hand_env.TwoHandsEnvBBeckman,
    ...
    """bbeckman: position and orientation are 'configuration variables' or
    'generalized coordinates' in Lagrangian mechanics. Normally, and for the
    robot, the shape of the configuration variables is exactly the same as the
    shape of their velocities, called 'generalized velocities.' Therefore, we
    do not need one name for the shape of the configuration variables and
    another name for the shape of their velocities in the case of the robot.
    For the cube, the configuration variables include a quaternion, which has
    one more dimension than its velocity (but not one more degree of freedom),
    so we need one name for the shape of the seven configuration variables and
    one name for their six velocities)."""
    ...
    def _reset_sim(self):
        ...
        # [[[ bbeckman: TODO: copy-paste code (CRG 21), but where from? ]]]
        if self.randomize_initial_rotation:
            if self.target_rotation == 'z':
                angle = self.np_random.uniform(-np.pi, np.pi)
                axis = np.array([0., 0., 1.])
    ...
    def _sample_goal(self):
        # This is called when the length of the episode, in time steps or in
        # wall-clock time, exceed set maxima. See 'envs.__init__.py' and
        # 'time_limit.py' for ways to change the maxima.
