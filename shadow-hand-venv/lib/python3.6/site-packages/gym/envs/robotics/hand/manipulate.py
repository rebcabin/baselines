...
class TwoHandsManipulateEnvBBeckman(hand_env.TwoHandsEnvBBeckman,
    ...
    def __init__(
        ...
        Args:
            ...
            distance_threshold (float, in meters): the threshold after which the position of a goal is considered achieved
            rotation_threshold (float, in radians): the threshold after which the rotation of a goal is considered achieved
            n_sub_steps (int): number of substeps the simulation runs on every call to step
            relative_control (boolean): whether or not the hand is actuated in absolute joint positions or relative to the current state
        """
        ...
        hand_env.TwoHandsEnvBBeckman.__init__(
            self, model_path, n_sub_steps=n_sub_steps, initial_qpos=initial_qpos,
            relative_control=relative_control)
        utils.EzPickle.__init__(self)
    ...
    # [[[ bbeckman: position and orientation are called 'configuration variables'
    # or 'generalized coordinates' in the language of Lagrangian mechanics.
    # Normally, and for the robot, the shape of the configuration variables is
    # exactly the same as the shape of their velocities. Therefore, we do not
    # need two names for the shapes of the configuration variables and for their
    # velocities in the case of the robot. For the cube, the configuration
    # variables include a quaternion, which has one more dimension than its
    # velocity (but not one more degree of freedom), so we need two names for
    # the shapes of the cube states).
    #
    # TODO: move these constants to a YAML or JSON config file.
    # ]]]
    ROBOT_CONFIGURATION_SHAPE = (24,)
    CUBE_CONFIGURATION_SHAPE = (7,)
    CUBE_VELOCITY_SHAPE = (6,)
    DOUBLE_ROBOT_CONFIG_SHAPE_DEPRECATED = (48,)

    def _get_obs(self):
        # [[[ bbeckman: new code here; changing things incrementally. "Left" means
        # from point-of-view of the camera, that is, the hand on the left when you
        # are looking at the screen. Likewise for "right." The left robot is
        # named "robot0" in the xml files in the assets folder. ]]]
        ...
        # [[[ bbeckman: "Rigt" is an abbreviation of "right" that has the same number
        # of letters as "left." This abbreviation makes code easier to read by
        # enhancing vertical alignment. I rarely use this abbreviation outside of
        # code because it might be confusing. The right robot is named "robot1" in
        # the xml files in the assets folder. ]]]
        ...
        # [[[ bbeckman: "object" means "cube" in our current application. The original
        # reuses this code for manipulating the egg and for manipulating the pen, so
        # they use the word "object" so as to be less specific. We retain that less
        ...
        # [[[ bbeckman: To maximize reuse and to minimize disruption in the XML files,
        # I left the name of the cube on the left-hand side of the screen as "object."
        # The cube on the right-hand side of the screen is called "object_right." The
        # object coordinates are assumed to be (x, y, z) and a 4D, normalized
        # quaternion, which really only has three degrees of freedom. "qpos" is, then
        # 7D, 6DOF. ]]]
        ...
        # [[[ bbeckman: velocity of the object is presumed to be (x_dot, y_dot, z_dot,
        # phi_roll_dot, theta_pitch_dot, psi_yaw_dot). In any event, it's 6D with six
        # degrees of freedom. ]]]
        ...
        # [[[ bbeckman: old code here. TODO: deprecate ]]]
        both_robots_qpos, robot_qvel = robot_get_obs(self.sim)
        assert both_robots_qpos.shape == robot_qvel.shape == \
            self.DOUBLE_ROBOT_CONFIG_SHAPE_DEPRECATED
        ...
        # [[[ bbeckman: To enhance incremental development, I add new information to
        # the old dict before deleting the old information. That way, I can check more
        # frequently and reduce the amount of time debugging. ]]]
        return {
            'observation': observation.copy(),
            'achieved_goal': achieved_goal.copy(),
            'desired_goal': self.goal.ravel().copy(),
...
class TwoHandsBlockEnvBBeckman(TwoHandsManipulateEnvBBeckman):
    """[[[ bbeckman: This is the top-level environment, mapped through
    the registry to the name 'TwoHandsManipulateBlocks-v0' in
    envs/__init__.py, line 415.

    'gym.make("TwoHandsManipulateBlocks-v0")' calls this constructor
    through wrapper layers. TODO: do we need changes to the wrappers?

    A copy so as not to derange the original. A
    ManipulateEnv(this file) is a HandEnv(hand_env.py), which is a
    RobotEnv(robot_env.py).
