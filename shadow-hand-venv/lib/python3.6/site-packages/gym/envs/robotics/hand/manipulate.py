...
class TwoHandsManipulateEnvBBeckman(hand_env.TwoHandsEnvBBeckman,
    ...
    Alternatively, we can track the five fingertip positions and velocities.
    ...
    ROBOT_FINGERTIP_POSITION_SHAPE = (15,)
    ROBOT_FINGERTIP_VEOLICITY_SHAPE = (15,)
    LEFT = 0   # audience-left, not stage-left
    RIGHT = 1  # audience-right, not stage-right
    ...
    def __init__(
        ...
        Args:
            ...
            randomize_initial_position (boolean): whether to randomize the
            initial position of the object

            randomize_initial_rotation (boolean): whether to randomize the
            ...
            n_sub_steps (int): number of sub-steps the simulation runs on every
            call to step
    ...
    def _get_achieved_goal(self):
        """Cube Cartesian [x y z] position in the first three slots of 'qpos,'
        4D normalized quaternion in the last four slots of 'qpos.' The name
    ...
    def _env_setup(self, initial_qpos):
        """bbeckman: init qpos comes in as an empty dict.
        Data format unknown."""
        for name, value in initial_qpos.items():
            self.sim.data.set_joint_qpos(name, value)
        # TODO: violation code-review guideline 9: magic number 0.5!
        # [[[ bbeckman: TODO: + 0.5 hack ]]]
        ...
        # That's one reason for leaving this magic number 0.5. ]]]
        # [[[ bbeckman: if initial_pos is the empty dict, then the initial qpos
        # comes from the robots_*.XML files in the assets directory. ]]]
        inspec0 = self.sim.data.get_joint_qpos("target:joint")
        self.sim.data.set_joint_qpos(
            "target_right:joint",
            inspec0 + [0.5, 0, 0, 0, 0, 0, 0]
        )
        self.sim.forward()  # Documentation for this API difficult to find.
        inspec1 = self.sim.data.get_joint_qpos("target:joint")
        # Target should not drift
        assert np.array_equal(inspec0, inspec1)
        return None
    ...
    def _reset_sim(self):
        ...
        # [[[ bbeckman: "cube, audience left, stage right. ]]]"
        initial_cube_left_qpos = \
            self.sim.data.get_joint_qpos('object:joint').copy()
        initial_cube_left_pos, initial_cube_left_quat = \
            initial_cube_left_qpos[:3], initial_cube_left_qpos[3:]

        assert initial_cube_left_qpos.shape == self.CUBE_CONFIGURATION_SHAPE
        assert initial_cube_left_pos.shape == self.POS_CONFIGURATION_SHAPE
        assert initial_cube_left_quat.shape == \
               self.QUATERNION_CONFIGURATION_SHAPE
        ...
        if self.randomize_initial_rotation:
            if self.target_rotation == 'z':
                ...
                axis = np.array([0., 0., 1.])
                offset_quat = quat_from_angle_and_axis(angle, axis)
                initial_cube_left_quat = \
                    rotations.quat_mul(initial_cube_left_quat, offset_quat)
            elif self.target_rotation == 'parallel':
                angle = self.np_random.uniform(-np.pi, np.pi)
                axis = np.array([0., 0., 1.])
                z_quat = quat_from_angle_and_axis(angle, axis)
                ...
                raise error.Error(
                    'Unknown target_rotation option "{}".'
                        .format(self.target_rotation))

        if self.randomize_initial_position:
            if self.target_position != 'fixed':
                initial_cube_left_pos += \
                    self.np_random.normal(size=3, scale=0.005)

        initial_cube_left_quat /= np.linalg.norm(initial_cube_left_quat)
        # [[[ bbeckman: TODO: 'qpos' is a regrettable name for pos + quat. ]]]
        initial_cube_left_qpos = \
            np.concatenate([initial_cube_left_pos, initial_cube_left_quat])
        self.sim.data.set_joint_qpos('object:joint', initial_cube_left_qpos)
    ...
    def _sample_goal(self):
        ...
        if self.target_position == 'random':
            ...
            assert offset.shape == (3,)
            target_pos = \
                self.sim.data.get_joint_qpos('object:joint')[:3] + offset
        elif self.target_position in ['ignore', 'fixed']:
            target_pos = \
                self.sim.data.get_joint_qpos('object:joint')[:3]
        else:
            raise error.Error(
                'Unknown target_position option "{}".'
        ...
        # [[[ bbeckman: violation code-review guideline #21: copy-paste! ]]]
    ...
    def _render_callback(self):
        ...
        inspect_qpos = self.sim.data.get_joint_qpos("target:joint")
        self.sim.data.set_joint_qpos(
            "target_right:joint",
            inspect_qpos + [0.5, 0, 0, 0, 0, 0, 0]
        )
        inspect_qvel = self.sim.data.get_joint_qvel("target:joint")
        self.sim.data.set_joint_qvel(
            'target_right:joint',
            inspect_qvel
        )

        # [[[ bbeckman: TODO: worry about object_right's being hidden ]]]
        if 'object_hidden' in self.sim.model.geom_names:
            hidden_id = self.sim.model.geom_name2id('object_hidden')
            self.sim.model.geom_rgba[hidden_id, 3] = 1.
    ...
    def _cartesian_fingertip_world_positions(self, robot_name_prefix):
        ...
        # TODO: Figure out to toggle rendering of group="3". See
        # TODO: commentary in assets/hand/robots.xml and robots_right.xml.
    ...
    def _get_obs(self):
        ...
        # "object_right." The object coordinates are (x, y, z) concatenated to
        # a 4D, normalized quaternion, which really only has three degrees of
        # freedom. "qpos" is, then 7D, 6DOF. ]]]
...
class TwoHandsBlockEnvBBeckman(TwoHandsManipulateEnvBBeckman):
    ...
    The class hierarchy follows:
            ...
            gym.envs.robotics.robot_env.TwoRobotsEnvBBeckman
             (.sim attribute in this class points to mujoco)
                                  ...
                                  is-a
                                   ||
                          in file 'hand_env.py'
             gym.envs.robotics.hand_env.TwoHandsEnvBBeckman
                                   /\
                                  is-a
                                   ||
                              in this file
      gym.envs.robotics.hand.manipulate.TwoHandsManipulateEnvBBeckman
        ...
        gym.envs.robotics.hand.manipulate.TwoHandsBlockEnvBBeckman
                              ...
                              instantiates
                                   ||
                    in top-level application or script
    ...
    TODO: make this environment a subclass of ManipulateEnv rather
    than a copy."""
    ...
    def __init__(self,
        ...
        super(TwoHandsBlockEnvBBeckman, self).__init__(
            ...
            target_rotation=target_rotation,
            target_position_range=np.array([(-0.04,  0.04),
                                            (-0.06,  0.02),
                                            ( 0.00,  0.06)]),
            reward_type=reward_type,
            seed=seed)
