...
class TwoHandsManipulateEnvBBeckman(hand_env.TwoHandsEnvBBeckman,
    ...
    def __init__(
        ...
        self.fingertip_names = [[n for n in self.sim.model.body_names
                                 if n.startswith('robot0') and
                                 n.endswith('distal')],
                                [n for n in self.sim.model.body_names
                                 if n.startswith('robot1') and
                                 n.endswith('distal')]]
    ...
    def _cartesian_world_poses(self, robot_name_prefix):

        # TODO: move this to __init__, not once every sim tick!
        body_names = [n for n in self.sim.model.body_names
                      if n.startswith(robot_name_prefix)]

        xposs = {'xpos:' + name: self.sim.data.get_body_xpos(name)
                 for name in body_names}

        # names = \
        #   ('world', 'floor0', 'robot0:hand mount',
        #    'robot0:forearm', 'robot0:wrist', 'robot0:palm',
        #    'robot0:ffknuckle',
        ...
        #    'robot0:lfmetacarpal', 'robot0:lfknuckle', 'robot0:lfproximal',
        #    'robot0:lfmiddle', 'robot0:lfdistal', 'robot0:thbase',
        #    'robot0:thproximal',
        #    'robot0:thhub', 'robot0:thmiddle', 'robot0:thdistal', 'object',
        #    'target')


        debug_cubie_visualization_of_fingertip_positions = True # [[[ bbeckman ]]]
        if debug_cubie_visualization_of_fingertip_positions:
            ffdistal = xposs['xpos:' + robot_name_prefix + ':ffdistal']
            lr = 'r' if robot_name_prefix[-1] == "1" else 'l'
            m = self.sim.model._body_name2id[lr + 'ffdistal:box']
            self.sim.model.body_pos[m][0] = ffdistal[0]
            self.sim.model.body_pos[m][1] = ffdistal[1]
            self.sim.model.body_pos[m][2] = ffdistal[2]

        debug_matplot_lib_visualization_of_fingertip_positions = False  # [[[ bbeckman ]]]
        if debug_matplot_lib_visualization_of_fingertip_positions:
            fig = plt.figure()
            ax = fig.add_subplot(111, projection='3d')
            for k, v in xposs.items():
                ax.scatter(*v, c='b' if k.endswith('distal') else 'r',
                           marker='^' if k.endswith('distal') else 'o')
            ax.set_xlabel('X world? ' + robot_name_prefix)
            ax.set_ylabel('Y world? ' + robot_name_prefix)
            ax.set_zlabel('Z world? ' + robot_name_prefix)
            plt.show()

        xquts = {'xqut:' + name: self.sim.data.get_body_xquat(name)
                 for name in body_names}

        xmats = {'xmat:' + name: self.sim.data.get_body_xmat(name)
                 for name in body_names}

        # xmocapposs = {'xmopos:' + name: sim.data.get_mocap_pos(name)
        #               for name in names}
        # xmocapquts = {'xmoqut:' + name: sim.data.get_mocap_quat(name)
        #               for name in names}

        return (xposs, xquts, xmats)
    ...
    def _get_obs(self):
        ...
        return {
            ...
            'left_cartesian_world_poses':
                self._cartesian_world_poses("robot0"),
            'rigt_cartesian_world_poses':
                self._cartesian_world_poses("robot1"),
        }
