...
class TwoHandsManipulateEnvBBeckman(hand_env.TwoHandsEnvBBeckman,
    ...
    def _get_obs(self):
        ...
        robot_rigt_qpos, robot_rigt_qvel = robot_get_obs(self.sim, "robot1")
        robot_qpos, robot_qvel = robot_get_obs(self.sim)
        assert(robot_qpos.shape == (48,))
        assert(robot_qvel.shape == (48,))
        object_qvel = self.sim.data.get_joint_qvel('object:joint')
        object_right_qvel = self.sim.data.get_joint_qvel('object_right:joint')  # [[[ bbeckman ]]]
        achieved_goal = self._get_achieved_goal().ravel()  # this contains the object position + rotation
        observation = np.concatenate([robot_qpos, robot_qvel, object_qvel, object_right_qvel, achieved_goal])
        return {
            'observation': observation.copy(),
            'achieved_goal': achieved_goal.copy(),
            'desired_goal': self.goal.ravel().copy(),
