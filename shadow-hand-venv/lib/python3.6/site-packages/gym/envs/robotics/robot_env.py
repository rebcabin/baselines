...
class TwoRobotsEnvBBeckman(gym.GoalEnv):
    ...
    def __init__(self,
                 ...
                 n_actions,
                 n_sub_steps):
        """[[[ bbeckman: 'initial_qpos comes in via kwargs from gym.make at top-level.
        For now, it's a harmless, empty dict."""
        if model_path.startswith('/'):
        ...
        self.seed()
        self._env_setup(initial_qpos=initial_qpos)  # [[[ bbeckman: one qpos for both targets ]]]
        # [[[ bbeckman: TODO: why is this array of shape (76,)? It should be 2 * (61 = 48 + 13) == 122.
        self.initial_state = copy.deepcopy(self.sim.get_state())

        self.goal = self._sample_goal()
        obs = self._get_obs()
        self.action_space = spaces.Box(-1., 1., shape=(n_actions,), dtype='float32')
        # [[[ bbeckman: this appears not to be used. ]]]
        self.observation_space = spaces.Dict(dict(
            desired_goal=spaces.Box(-np.inf, np.inf, shape=obs['achieved_goal'].shape, dtype='float32'),
            achieved_goal=spaces.Box(-np.inf, np.inf, shape=obs['achieved_goal'].shape, dtype='float32'),
            observation=spaces.Box(-np.inf, np.inf, shape=obs['observation'].shape, dtype='float32'),
    ...
    def step(self, action):
        ...
        double_goal = np.concatenate([
            self.goal,  # left-hand cube
            self.goal + [0.5, 0, 0, 0, 0, 0, 0]  # right-hand cube
        ])
        info = {
            'is_success': self._is_success(
                obs['achieved_goal'],  # [[[ bbeckman: already doubled ]]]
                double_goal),
        }
        reward = self.compute_reward(obs['achieved_goal'], double_goal, info)
        return obs, reward, done, info
